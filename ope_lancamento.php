<?php
	...
	$tpl->addFile("JSCRIPT", 'cssjs_libs/js_parts/js_ope_lancamento.html');
	$tpl->addFile("CONTEUDO", 'html_libs/ope_lancamento-form.html');
		...
		***************************************/	
		...
		if(isset($_POST['nota'],$_POST['emissao'],$_POST['vencto'],$_POST['valor']) && $rgi != "" && $mes_ref != "" && $ano_ref != ""){
			...
			if($rgi <= 0){
				...
				$invalidos++;
				$alerta.= $invalidos . " - Campo Registro de UC deve ser preenchido obrigatoriamente.<br />";
	...
	***************************************************/
	...
	if(isset($_GET['id']) && $_GET['id'] > 0){
		...
		$id = (int) $_GET['id'];
		$sql_verif	= "SELECT * FROM daee_uddc WHERE id=$id AND ativo=1";
		if(mysql_num_rows(mysql_query($sql_verif)) == 1){
				
			$uc = new UnidadeConsumidora($id);
			$tpl->RGID		= $tpl->HIDERGID = $id;
			$tpl->RGI		= $uc->get('rgi');
			$tpl->RGINOME	= $uc->getEndereco() . " ~> " .$uc->getNome();
			$tpl->DISABLE_RGI = 'disabled="disabled"';
			$tpl->block('EACH_RGI');
