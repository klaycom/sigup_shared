...
class Template {
	...
	private function subst($varname) {
		...
		foreach($this->instances as $varname => $inst) foreach($this->properties[$varname] as $p){
			...
			elseif(method_exists($inst, "__get")) $s = str_replace("{".$varname.'->'.$p."}", $inst->__get($p), $s);
			else throw new BadMethodCallException("não existe método na classe ".get_class($inst)." para acessar ".$varname."->".$p);
		}
		return $s;
	}
